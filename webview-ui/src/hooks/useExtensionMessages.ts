  ...
  useEffect(() => {
    ...
    const handler = (msg: any) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        // If the layout is already loaded, add agents immediately; otherwise buffer
        // them so layoutLoaded can add them once seats are built. existingAgents can
        // arrive either before or after layoutLoaded (WS message order isn't
        // guaranteed), so relying solely on the buffer drops agents whenever
        // existingAgents arrives last.
